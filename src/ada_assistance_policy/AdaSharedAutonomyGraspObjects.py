...
def Init_Robot(robot):
    ...
    # if (self.sim):
    robot_pose = np.array([[1, 0, 0, 0.409], [0, 1, 0, 0.338], [
                          0, 0, 1, 0.795], [0, 0, 0, 1]])
...
def Finish_Trial_Func(ada_handler):
    print('Finishing trial')
    robot = ada_handler.robot
    if robot.simulated:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--user-sim', action='store_true')
    ...
    parser.add_argument('-input', '--input-interface-name',
                        help='name of the input interface. Possible choices: ' + str(possible_teleop_interface_names), type=str)
    parser.add_argument('-joy_dofs', '--num-input-dofs',
                        help='number of dofs of input, either 2 or 3', type=int, default=2)
    ...
    env_path = os.path.join(
        path_to_pkg, 'data', 'environments', 'tablewithobjects_assisttest.env.xml')

    env, robot = Initialize_Adapy(args, env_path=env_path)

    for i in range(1):
        Reset_Robot(robot)
        goals, goal_objects = Initialize_Goals(
            env, robot, randomize_goal_init=False)
        ada_handler = AdaHandlerFuture(env, robot,
                                       AdaHandlerConfig.create(
                                           goals=goals, goal_objects=goal_objects, input_interface_name=args.input_interface_name, 
                                           num_input_dofs=args.num_input_dofs, use_finger_mode=False),
                                       PolicyConfig.create(simulate_user=args.user_sim, direct_teleop_only=True, 
                                            fix_magnitude_user_command=False))
        ada_handler.add_done_callback(Finish_Trial_Func)

    def do_cancel(_):
        ada_handler.cancel()

    # timer = rospy.Timer(rospy.Duration(5.), do_cancel, oneshot=True)
